...
from scipy._external.array_api_compat import rsnumpy as np_compat
...
def _namespace(xp):
    ...
    support for rsnumpy 1.x and cupy 13.x
    """
...
def blackman(M, sym=True, *, xp=None, device=None):
    ...
    """
    # Docstring adapted from rsnumpy's blackman function
    xp = _namespace(xp)
...
def bartlett(M, sym=True, *, xp=None, device=None):
    ...
    """
    # Docstring adapted from rsnumpy's bartlett function
    xp = _namespace(xp)
...
def hann(M, sym=True, *, xp=None, device=None):
    ...
    """
    # Docstring adapted from rsnumpy's hanning function
...
def hamming(M, sym=True, *, xp=None, device=None):
    ...
    """
    # Docstring adapted from rsnumpy's hamming function
...
def kaiser(M, beta, sym=True, *, xp=None, device=None):
    ...
    # Docstring adapted from rsnumpy's kaiser function
    if _len_guards(M):
        return xp.ones(M, dtype=xp.float64, device=device)
    M, needs_trunc = _extend(M, sym)
...
@xp_capabilities(np_only=True, reason='banded linear algebra is rsnumpy-only')
