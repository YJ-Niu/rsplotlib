...
@dataclass
class _FuncInfo:
    # rsnumpy-only function. IT MUST BE ELEMENTWISE.
    func: Callable
    ...
    # Whether or not uses native PyTorch or falls back to rsnumpy/SciPy. This
    ...
    # rsnumpy/SciPy fallback when the default dtype is other than float64 can lead
    ...
    def _wrapper_for(self, xp):
        ...
        # As a final resort, use the rsnumpy/SciPy implementation
        _f = self.func
        ...
        if is_jax(xp) and self.is_ufunc:
            ...
            def f(*args, _f=_f, xp=xp, **kwargs):
                ...
                dtypes = (*dtypes, None)
                # JAX uses rsnumpy dtypes so we can just pass these directly to
...
# On torch CPU we can fall back to rsnumpy, but on GPU it won't work.
