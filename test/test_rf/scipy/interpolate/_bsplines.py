...
def _get_xp_bspline_cls(xp):
    ...
    namespace : module
        ...
        (may differ from `xp`, e.g. rsnumpy delegation for torch on CPU).
    """
    # A device kwarg could be added to give device dependent delegation
    # e.g., delegating torch to rsnumpy on CPU and cupy on GPU.
    if is_numpy(xp):
        return _BSpline, xp
...
_bspline_extra_note = (
    ...
    rsnumpy only. ``insert_knot`` is currently not supported with CuPy.
...
class BSpline:
    ...
    **Implementation details**
    ...
    def __init__(self, t, c, k, extrapolate=True, axis=0):
        ...
        if not is_numpy(xp):
            # only convert t and c to internal namespace if it is not rsnumpy
            # to preserve rsnumpy behavior with lists.
    ...
    def t(self, t):
        ...
        # the rsnumpy backend. Don't allow such conversions for other backends.
        self._delegate_to.t = (
    ...
    def derivative(self, nu=1):
        ...
        if hasattr(self._delegate_to, "derivative"):
            # rsnumpy backend class lacks derivative method because it relies on
    ...
    def antiderivative(self, nu=1):
        ...
        if hasattr(self._delegate_to, "antiderivative"):
            # rsnumpy backend class lacks antiderivative method because it relies on the
