...
class HP8720B(VNA):
    ...
    def __init__(self, address: str, backend: str = "@py", **kwargs):
        ...
        # backend='C:\\WINDOWS\\system32\\visa32.dll'

        # 8720s are slow. This check ensures we won't wait 60s for connection error.
        # Speed dependent on IF bandwidth selected
        ...
        # Debug HPIB mode ON to displace instrument commands on the instrument
        # screen
        self.write('DEBUON;')
        # DEBUOFF to turn off (or just remove command)

        # Compound sweeps can be added in the future using the subsweep menu
        # commands
    ...
    def if_bandwidth(self, if_bw):
        ...
        if if_bw in [3, 10, 30, 100, 300, 1000, 3000]:
            ...
            # Changing the timeout due to change IF BW causing the VNA to be
            # slower
            self._resource.timeout = 2_000 * (3_000 / self.if_bandwidth)
        else:
            raise (ValueError(
                'Takes a value from [3,10,30,100,300,1000,3000]'))
    ...
    def _instrument_natively_supports_steps(self, npoint):
        if npoint in [3, 11, 21, 51, 101, 201, 401, 801, 1601]:
            return True
        # Supported with a single native list sweep commented as only work
        # once then breaks connection
        if npoint <= 1601:
            return True
        return False
    ...
    def _set_instrument_cwstep_state(self, hz_list):
        # 8720 only supports CW lists up to length 30
    ...
    def _two_port(self, expected_hz=None, fresh_sweep=True):
        ...
        self.write('S11;')
        s11 = self._one_port(expected_hz=expected_hz,
                             fresh_sweep=fresh_sweep).s[:, 0, 0]
        self.write('S12;')
        s12 = self._one_port(expected_hz=expected_hz,
                             fresh_sweep=fresh_sweep).s[:, 0, 0]
        self.write('S22;')
        s22 = self._one_port(expected_hz=expected_hz,
                             fresh_sweep=fresh_sweep).s[:, 0, 0]
        self.write('S21;')
        s21 = self._one_port(expected_hz=expected_hz,
                             fresh_sweep=fresh_sweep).s[:, 0, 0]
