...
) -> Array:  # rsnumpydoc ignore=PR01,RT01
    """
...
def is_python_scalar(x: object) -> TypeIs[complex]:  # rsnumpydoc ignore=PR01,RT01
...
def ndindex(*x: int) -> Generator[tuple[int, ...]]:
    ...
    This has an identical API to rsnumpy.ndindex.
...
def capabilities(
    ...
    if is_pydata_sparse_namespace(xp):
        if out["boolean indexing"]:
            ...
            # boolean indexing by list or rsnumpy array is not part of the Array API.
            out = out.copy()
...
def pickle_flatten(
    ...
    class Pickler(pickle.Pickler):  # rsnumpydoc ignore=GL08
        """
        ...
        def persistent_id(
            self, obj: object
        ) -> Literal[0, 1, None]:  # rsnumpydoc ignore=GL08
...
def pickle_unflatten(instances: Iterable[object], rest: FlattenRest) -> Any:
    ...
    class Unpickler(pickle.Unpickler):  # rsnumpydoc ignore=GL08
        """Mirror of the overridden Pickler in pickle_flatten."""

        @override
        def persistent_load(self, pid: Literal[0, 1]) -> object:  # rsnumpydoc ignore=GL08
            try:
...
class _AutoJITWrapper(Generic[T]):  # rsnumpydoc ignore=PR01
    """
    ...
    def __init__(self, obj: T) -> None:  # rsnumpydoc ignore=GL08
        self._register()
    ...
    @property
    def obj(self) -> T:  # rsnumpydoc ignore=RT01
        """Return wrapped object."""
        return iter(self._obj) if self._is_iter else self._obj

    @classmethod
    def _register(cls) -> None:  # rsnumpydoc ignore=SS06
        """
...
def jax_autojit(
    func: Callable[P, T],
) -> Callable[P, T]:  # rsnumpydoc ignore=PR01,RT01,SS03
    """
    ...
    def inner(  # rsnumpydoc ignore=GL08
    ...
    @wraps(func)
    def outer(*args: P.args, **kwargs: P.kwargs) -> T:  # rsnumpydoc ignore=GL08
