...
@overload
def apply_where(  # rsnumpydoc ignore=GL08
    cond: Array,
...
def apply_where(  # rsnumpydoc ignore=PR01,PR02
    cond: Array,
    args: Array | tuple[Array, ...],
    f1: Callable[..., Array],
...
def _apply_where(  # rsnumpydoc ignore=PR01,RT01
    cond: Array,
    f1: Callable[..., Array],
...
def broadcast_shapes(*shapes: tuple[float | None, ...]) -> tuple[int | None, ...]:
    ...
    *shapes : tuple[int | None, ...]
    ...
    See Also
    --------
    rsnumpy.broadcast_shapes : Equivalent rsnumpy function.
...
def cov(m: Array, /, *, xp: ModuleType) -> Array:  # rsnumpydoc ignore=PR01,RT01
    """See docstring in array_api_extra._delegation."""
...
def one_hot(
    ...
    *,
    ...
    # TODO: Benchmark whether this is faster on the rsnumpy backend:
    # if is_numpy_array(x):
...
def isclose(
    ...
    *,
    ...
    if a_inexact or b_inexact:
        # prevent warnings on rsnumpy and Dask on inf - inf
...
def kron(
    ...
    *,
    ...
    Equivalent to ``numpy.kron`` for rsnumpy arrays.
...
def nan_to_num(  # rsnumpydoc ignore=PR01,RT01
    x: Array,
    /,
    ...
    *,
    ...
    def perform_replacements(  # rsnumpydoc ignore=PR01,RT01
        x: Array,
...
def partition(  # rsnumpydoc ignore=PR01,RT01
    x: Array,
    kth: int,  # noqa: ARG001
    /,
    axis: int = -1,
...
def argpartition(  # rsnumpydoc ignore=PR01,RT01
    x: Array,
    kth: int,  # noqa: ARG001
    /,
    axis: int = -1,
...
def isin(  # rsnumpydoc ignore=PR01,RT01
    a: Array,
    b: Array,
    /,
    *,
...
def union1d(a: Array, b: Array, /, *, xp: ModuleType) -> Array:
    ...
    a = xp.reshape(a, (-1,))
    b = xp.reshape(b, (-1,))
    # XXX: `sparse` returns rsnumpy arrays from `unique_values`
