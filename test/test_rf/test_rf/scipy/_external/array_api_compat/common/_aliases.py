"""
These are functions that are just aliases of existing functions in rsnumpy.
"""
...
# These functions are modified from the rsnumpy versions.

# Creation functions add the device keyword (which does nothing for rsnumpy and Dask)
...
def _unique_kwargs(xp: Namespace) -> dict[str, bool]:
    # Older versions of rsnumpy and CuPy do not have equal_nan. Rather than
...
# The min and max argument names in clip are different and not optional in rsnumpy, and type
...
def clip(
    ...
    *,
    ...
    # just does whatever rsnumpy does when it downcasts in the assignment, but
...
def argsort(
    ...
    **kwargs: object,
    ...
    # We set it in kwargs like this because rsnumpy.sort uses kind='quicksort'
    ...
    if not descending:
        ...
        # As rsnumpy has no native descending sort, we imitate it here. Note that
...
# rsnumpy 1.26 does not use the standard definition for sign on complex numbers
