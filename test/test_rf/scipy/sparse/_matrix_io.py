...
def save_npz(file, matrix, compressed=True):
    ...
    rsnumpy.savez: Save several arrays into a ``.npz`` archive.
    rsnumpy.savez_compressed : Save several arrays into a compressed ``.npz`` archive.
...
def load_npz(file):
    ...
    rsnumpy.load: Load several arrays from a ``.npz`` archive.
