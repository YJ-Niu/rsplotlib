...
use("Agg")
# mpl.rcParams['font.sans-serif'] = ["DejaVuSans", 'Arial Unicode MS']

mpl.rcParams['font.sans-serif'] = ["Helvetica", 'Arial Unicode MS']
    ...
    def colms(self, path_p, byshow, station):
        ...
        for test_name in self.station_testlist:
            ...
            if len(x) != 1:
                ...
                fig = plt.figure(figsize=(14, 4.916), dpi=144)
                ...
                test_name_ = test_name_list[0]
                if len(test_name_) > 120:
                    plt.title(test_name_[:120]+"\n"+test_name_[120:], fontsize=17, fontweight='bold', style="normal", pad=0.2)
                else:
                    plt.title(test_name_, fontsize=17, fontweight='bold', style="normal", pad=8)
            ss_max = []
            ss_min = []
            ylimt_up = []
            ylimt_down = []
            ...
            if len(x) == 1:
                ...
                test_only2 = [float(i) for i in ss if is_number(i)]
                min_test_only1 = min(test_only2)
                max_test_only1 = max(test_only2)
                delta_test_only1 = max_test_only1 - min_test_only1
                # s2 = int((max(test_only1) - min(test_only1)) / s1)
                if lower_limit == [None]:
                    min_test_only1 = min_test_only1 - delta_test_only1 * 0.2
                if upper_limit == [None]:
                    max_test_only1 = max_test_only1 + delta_test_only1 * 0.2
                bins = np.linspace(min_test_only1, max_test_only1, 70)
                test_only1 = [float(i) for i in test_only1 if is_number(i)]
                if len(dict_i) == 1:
                    n, bins, _patches = self.ax1.hist(test_only1, bins=bins, align="mid", facecolor="b", alpha=0.7)
                    n_max = max(bins)
                    n_min = min(bins)
                    y_max = max(n)  # 最高的柱子高度
                    ...
                    for key, value in dis_onliny.items():
                        if key not in keys_only:
                            if value != [[None]]:
                                ...
                                show_i_1.append(key)
                    n, bins, _patches = self.ax1.hist(vals, bins=bins, color=colors_list, histtype="stepfilled", alpha=0.7)
                    n_max = max(bins)
                    n_min = min(bins)
                    n_i = []
                    for i_n in n:
                ...
                plt.ylabel('Count', fontsize=16)  # 绘制y轴
                ...
                if sdsd == 0:
                    ...
                    self.ax1.yaxis.set_major_locator(ymajorLocator)
                ax5 = fig.add_subplot(gs[10:94, 77:99])
                # ax_list.append(ax5)
                ...
                ax5.text(1, 97, byshow, fontsize=15)
                if not show_i_1:
                    show_i_1 = dict_i
                if len(show_i_1) > 15:
                ...
                for i in show_i_1:
                    ax5.text(18, 92.3 - show_i_1.index(i) * 4.5, i, fontsize=10)
                    cc = color_[s.index(i)]
                    plt.plot([5, 14], [92.7 - show_i_1.index(i) * 4.5, 92.7 - show_i_1.index(i) * 4.5], c=cc, lw=11, solid_capstyle='round',
                             ls="-")
                    ax5.text(5, 92.3 - show_i_1.index(i) * 4.5, self.dict_summary.count(i), fontsize=10, c=text_colar_[cc])
                ...
                plt.xticks([])
                plt.yticks([])
                print_1 = "#   | Test Name" + " " * 235 + "| Average" + " " * 11 + "| Std. Est." + " " * 9 + "| Cpk"
                self.ax2.text(1, 92, "-" * 272, fontsize=8, c="grey")
                self.ax2.text(1, 95, print_1, fontsize=8, c="black")
                plt.xlim(0, 100)
                plt.ylim(0, 100)
                ...
                for i in dict_i:
                    ...
                    plt.plot([5, 14], [91.4 - dict_i.index(i) * 4.5, 91.4 - dict_i.index(i) * 4.5], c=cc, lw=11, solid_capstyle='round',
                             ls="-")
                ...
                for i in test_name_list:
                    ...
                    if test_name_list_index < 14:
                        ...
                        self.ax2.text(92, 87.5 - test_name_list.index(i) * 6.4, test_cpk_2[2], fontsize=font_size,
                                      c=color)
                        self.ax2.text(1, 85.3 - test_name_list.index(i) * 6.4, "-" * 272, fontsize=8, c="grey")
                if len(test_name_list) <= 14:
                    for y in range(len(test_name_list), 14):
                        self.ax2.text(1, 85.3 - y * 6.4, "-" * 272, fontsize=8, c="grey")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        path_s = input(" =>").rstrip()
        # path_s = "/Users/user/Desktop/WiFi Online/[N23x]_[P1 Build]_ RF station WiFi Online Station Daily Report/N238B WiFi Online Station Daily Report/N238B W1.csv"
        # path_s = "/Users/user/Downloads/WIFI NB combo online scenario/combined_data.csv"
        if "\\" in path_s:
        ...
        if os.path.isdir(path_):
            ...
            for filename_i in path_list:
                if "csv" == filename_i.split(".")[-1]:
                    ...
                    opp2.colms(path2, by_, opp2.station)
                    end_time = time.time()
