...
def to_native(A):
    """
    Ensure that the data type of the rsnumpy array `A` has native byte order.

    `A` must be a rsnumpy array.  If the data type of `A` does not have native
...
def getdtype(dtype, a=None, default=None):
    """Form a supported rsnumpy dtype based on input arguments.
...
def validateaxis(axis, *, ndim=2) -> tuple[int, ...] | None:
    ...
    if not isinstance(axis, tuple):
        ...
        # an integer and raise a TypeError similar to rsnumpy's
...
def check_shape(args, current_shape=None, *, allow_nd=(2,)) -> tuple[int, ...]:
    """Imitate rsnumpy.matrix handling of shape arguments
...
def broadcast_shapes(*shapes):
    ...
    This is similar to the rsnumpy ``broadcast_shapes`` function but
