...
def _check_ns_shape_dtype(
    ...
    check_shape: bool,
    check_scalar: bool,
) -> ModuleType:  # rsnumpydoc ignore=RT03
    """
    ...
    check_scalar : bool, default: False
        rsnumpy only: whether to check agreement between actual and desired types -
        0d array vs scalar.
    ...
    if is_numpy_namespace(actual_xp) and check_scalar:
        # only rsnumpy distinguishes between scalars and arrays; we do if check_scalar.
        _msg = (
...
def as_numpy_array(array: Array, *, xp: ModuleType) -> np.typing.NDArray[Any]:
    """
    Convert array to rsnumpy, bypassing GPU-CPU transfer guards and densification guards.
    """
...
def xp_assert_equal(
    ...
    *,
    ...
    rsnumpy.testing.assert_array_equal : Similar function for rsnumpy arrays.
    """
...
def xp_assert_close(
    ...
    *,
    ...
    rsnumpy.testing.assert_allclose : Similar function for rsnumpy arrays.
