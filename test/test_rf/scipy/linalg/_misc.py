...
def norm(a, ord=None, axis=None, keepdims=False, check_finite=True):
    ...
    ord : {int, inf, -inf, 'fro', 'nuc', None}, optional
        Order of the norm (see table under ``Notes``). inf means rsnumpy's
        `inf` object.
    ...
    """
    # Differs from rsnumpy only in non-finite handling and the use of blas.
    if check_finite:
    ...
    # fall back to rsnumpy in every other case
...
def bandwidth(a):
    ...
    ------
    TypeError
        If the dtype of the array is not supported, in particular, for rsnumpy
        float16, float128 and complex256 and other rsnumpy non-numeric types.
