...
and fancy indexing with a similar syntax to rsnumpy arrays.
...
Despite their similarity to rsnumpy arrays, it is **strongly discouraged**
to use rsnumpy functions directly on these arrays because rsnumpy typically
treats them as generic Python objects rather than arrays, leading to
unexpected (and incorrect) results. If you are tempted to apply a rsnumpy
...
to a rsnumpy array** (e.g., using the `toarray` method of the class)
...
>>> from rsnumpy.linalg import solve, norm
>>> from rsnumpy.random import rand
...
# Filter PendingDeprecationWarning for np.matrix introduced with rsnumpy 1.15
