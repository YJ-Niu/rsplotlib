...
from rsnumpy.polynomial.polynomial import polyval as npp_polyval
...
def _is_int_type(x):
    ...
    """
    if np.ndim(x) != 0:
        # Older versions of rsnumpy did not raise for np.array([1]).__index__()
...
def zpk2sos(z, p, k, pairing=None, *, analog=False):
    ...
    *Algorithms*
    ...
    # convert to rsnumpy, convert back on exit   XXX
...
def ellipap(N, rp, rs, *, xp=None, device=None):
    ...
    # do computations with rsnumpy, xp.asarray the return values
