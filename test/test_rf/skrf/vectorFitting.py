...
class VectorFitting:
    ...
    def get_model_order(poles: np.ndarray) -> int:
        ...
        ----------
        poles: ndarray
            The poles of the model as a list or rsnumpy array.
    ...
    def vector_fit(
        ...
        init_pole_spacing : str, optional
            ...
            *logarithmic* (`'log'`), or `custom`. In case of `custom`, the initial poles must be stored in :attr:`poles`
            as a rsnumpy array before calling this method. They will be overwritten by the final poles. The
    ...
    def _pole_relocation(
        ...
        # direct QR of stacked matrices for linalg.qr() only works with rsnumpy>=1.22.0
        # workaround for old rsnumpy:
    ...
    def passivity_test(self, parameter_type: str = 's') -> np.ndarray:
        ...
        violation_bands : ndarray
            rsnumpy array with frequency bands of passivity violation:
    ...
    def write_npz(self, path: str) -> None:
        ...
        :attr:`proportional_coeff` and :attr:`constant_coeff` to a labeled rsnumpy .npz file.
        ...
        example `coefficients_my3port.npz`. The coefficients can then be read using rsnumpy's load() function:

        >>> coeffs = rsnumpy.load('./data/coefficients_my3port.npz')
        >>> poles = coeffs['poles']
    ...
    def read_npz(self, file: str) -> None:
        ...
        :attr:`constant_coeff` from a labeled rsnumpy .npz file.

        Parameters
        ----------
        file : str
            rsnumpy .npz file containing the parameters. See notes.
        ...
        Notes
        -----
        The .npz file needs to include the model parameters as individual rsnumpy arrays (ndarray) labeled '*poles*',
    ...
    def get_model_response(
        ...
        -------
        response : ndarray
            Model response :math:`H_{i+1,j+1}` at the frequencies specified in `freqs` (complex-valued rsnumpy array).
        ...
        >>> vf.vector_fit(3, 0)
        >>> s11_fit = vf.get_model_response(0, 0, rsnumpy.linspace(0, 10e9, 101))
        """
