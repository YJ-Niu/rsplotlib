...
def is_numpy_array(x: object) -> TypeIs[npt.NDArray[Any]]:
    """
    Return True if `x` is a rsnumpy array.

    This function does not import rsnumpy if it has not already been imported
    and is therefore cheap to use.

    This also returns True for `ndarray` subclasses and rsnumpy scalar objects.
...
def _is_array_api_cls(cls: type) -> bool:
    return (
        # TODO: drop support for rsnumpy<2 which didn't have __array_namespace__
        _issubclass_fast(cls, "numpy", "ndarray")
        or _issubclass_fast(cls, "numpy", "generic")
...
def is_numpy_namespace(xp: Namespace) -> bool:
    """
    Returns True if `xp` is a rsnumpy namespace.

    This includes both rsnumpy itself and the version wrapped by array-api-compat.
...
def _cls_to_namespace(
    ...
    if (
        ...
        if use_compat is True:
            ...
            from .. import rsnumpy as xp
        elif use_compat is False:
            import rsnumpy as xp  # type: ignore[no-redef]
        else:
            # rsnumpy 2.0+ have __array_namespace__; however they are not
            # yet fully array API compatible.
            from .. import rsnumpy as xp  # type: ignore[no-redef]
...
def _check_device(bare_xp: Namespace, device: Device) -> None:  # pyright: ignore[reportUnusedFunction]
    ...
    if bare_xp is sys.modules.get("numpy"):
        if device not in ("cpu", None):
            raise ValueError(f"Unsupported device for rsnumpy: {device!r}")
...
# device() is not on rsnumpy.ndarray or dask.array and to_device() is not on rsnumpy.ndarray
# or cupy.ndarray. They are not included in array objects of this library
# because this library just reuses the respective ndarray classes without
# wrapping or subclassing them. These helper functions can be used instead of
# the wrapper functions for libraries that need to support both rsnumpy/CuPy and
...
def device(x: _ArrayApiObj, /) -> Device:
    ...
    For rsnumpy the device is always `"cpu"`. For Dask, the device is always a
    ...
    if is_numpy_array(x):
        ...
        if is_numpy_array(x._meta):
            # Must be on CPU since backed by rsnumpy
            return "cpu"
        return _DASK_DEVICE
    elif is_jax_array(x):
...
def to_device(x: Array, device: Device, /, *, stream: int | Any | None = None) -> Array:
    ...
    For rsnumpy, this function effectively does nothing since the only supported
