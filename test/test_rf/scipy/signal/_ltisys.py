...
class StateSpace(LinearTimeInvariant):
    ...
    *system : arguments
    ...
    by calling `abcd_normalize`. The resulting dtype will be based on rsnumpy's dtype
    ...
    # Override rsnumpy binary operations and ufuncs
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, np.ndarray) and other.ndim > 0:
            ...
            raise ValueError("Cannot divide StateSpace by non-scalar rsnumpy arrays")
...
def _YT_real(ker_pole, Q, transfer_matrix, i, j):
    ...
    # VM is V.T with rsnumpy we want the first two lines of V.T
...
def _YT_loop(ker_pole, transfer_matrix, poles, B, maxiter, rtol):
    ...
    # Stick to the indices in the paper and then remove one to get rsnumpy array
