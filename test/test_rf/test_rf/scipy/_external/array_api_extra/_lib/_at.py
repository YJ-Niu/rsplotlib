...
class at:  # pylint: disable=invalid-name  # rsnumpydoc ignore=PR02
    """
    Update operations for read-only arrays.
    ...
    If x is a read-only rsnumpy array, they are the same as::
    ...
    ) -> None:  # rsnumpydoc ignore=GL08
        self._x = x
        self._idx = idx

    def __getitem__(self, idx: SetIndex, /) -> Self:  # rsnumpydoc ignore=PR01,RT01
        """
    ...
    def _op(
        ...
        if copy or (copy is None and not writeable):
            ...
            # A copy of a read-only rsnumpy array is writeable
    ...
    def min(
        ...
        # Note that da.minimum _incidentally_ works on rsnumpy, CuPy, and sparse
