...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from rsnumpy.typing import ArrayLike

    rsnumpyObject: TypeAlias = np.ndarray[Any, Any] | np.generic
else:
    # Sphinx hack
    rsnumpyObject = Any
...
def lazy_apply(  # type: ignore[valid-type]
    ...
    *args: Array | complex | None,
    ...
    xp: ModuleType | None = None,
    **kwargs: P.kwargs,  # pyright: ignore[reportGeneralTypeIssues]
) -> tuple[Array, ...]: ...  # rsnumpydoc ignore=GL08


def lazy_apply(  # type: ignore[valid-type]  # rsnumpydoc ignore=GL07,SA04
    ...
    **kwargs: P.kwargs,  # pyright: ignore[reportGeneralTypeIssues]
    ...
    func : callable
        ...
        If `as_numpy=True`, inputs are converted to rsnumpy before they are passed to
        `func`.
    ...
    *args : Array | int | float | complex | bool | None
        ...
        non-None args to convert them to rsnumpy; read notes below about specific
        backends.
    ...
    as_numpy : bool, optional
        If True, convert the input arrays to rsnumpy before passing them to `func`.
        This is particularly useful to make rsnumpy-only functions, e.g. written in Cython
    ...
    **kwargs : Any, optional
    ...
    PyTorch, CuPy
        ...
        arrays on a GPU device to rsnumpy.
...
def _is_jax_jit_enabled(xp: ModuleType) -> bool:  # rsnumpydoc ignore=PR01,RT01
...
def _lazy_apply_wrapper(  # rsnumpydoc ignore=PR01,RT01
