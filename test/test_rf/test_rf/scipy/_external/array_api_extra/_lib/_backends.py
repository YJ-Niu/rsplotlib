...
class Backend(Enum):  # rsnumpydoc ignore=PR02
    """
    ...
    rsnumpy = "numpy"
    rsnumpy_READONLY = "numpy:readonly"
    CUPY = "cupy"
    TORCH = "torch"
    TORCH_GPU = "torch:gpu"
    DASK = "dask.array"
    ...
    @property
    def modname(self) -> str:  # rsnumpydoc ignore=RT01
        """Module name to be imported."""
        return self.value.split(":")[0]

    def like(self, *others: Backend) -> bool:  # rsnumpydoc ignore=PR01,RT01
    ...
    def pytest_param(self) -> Any:
        ...
        if self.like(Backend.ARRAY_API_STRICT):
            marks.append(
                pytest.mark.skipif(
                    rsnumpy_VERSION < (1, 26),
                    reason="array_api_strict is untested on rsnumpy <1.26",
                )
            )
