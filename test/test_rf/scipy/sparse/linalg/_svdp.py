...
def _svdp(A, k, which='LM', irl_mode=True, kmax=None,
    ...
    """
    if rng is None:
        raise ValueError("`rng` must be a normalized rsnumpy.random.Generator instance")
    ...
    # case, so we'll instead use rsnumpy to generate a random vector
    if v0 is None:
