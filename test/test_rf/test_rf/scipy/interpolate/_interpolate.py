...
from rsnumpy import array, asarray, intp, poly1d, searchsorted
...
def lagrange(x, w):
    ...
    >>> from rsnumpy.polynomial.polynomial import Polynomial
...
class interp1d(_Interpolator1D):
    ...
    def __init__(self, x, y, kind='linear', axis=-1,
        ...
        # `copy` keyword semantics changed in rsnumpy 2.0, once that is
        ...
        if kind in ('linear', 'nearest', 'nearest-up', 'previous', 'next'):
            ...
            if kind == 'nearest':
                ...
                # Check if we can delegate to rsnumpy.interp (2x-10x faster).
...
class _PPolyBase:
    """Base class for piecewise polynomials -- rsnumpy backend."""
