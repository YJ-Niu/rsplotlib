...
underlying `xp` library, rsnumpy in particular is inconsistent in its handling
...
of the inconsistencies present in rsnumpy, as well as 20+ years of code on top,
...
However, when scalars are not generally the expected rsnumpy return type,
...
def _check_scalar(actual, desired, *, xp=None, **kwargs):
    ...
    # Only rsnumpy distinguishes between scalars and arrays;
    # shape check in xp_assert_* is sufficient except for shape == ()
    if not (is_numpy(xp) and desired.shape == ()):
        return

    _msg = ("Result is a rsnumpy 0d-array. Many SciPy functions intend to follow "
            "the convention of many rsnumpy functions, returning a scalar when a "
