...
def read_pna_csv(filename, *args, **kwargs) -> tuple[str, str, np.ndarray]:
    ...
    with open(filename) as fid:
        ...
        for k, line in enumerate(fid.readlines()):
            if line.startswith('!'):
                ...
                if n_END == 0:
                    # first END spotted -> set end_line to read first data
                    # block only
                    end_line = k
    ...
    units_dict: dict[str, FrequencyUnitT] = {
        k.lower(): k for k in FREQ_UNITS.keys()}
    ...
    try:
        ...
        raise ValueError(
            f"Could not parse frequency unit '{unit_raw}'") from exc
...
def pna_csv_2_df(filename):
    ...
    df = DataFrame({names[k]: d[:, k]
                   for k in range(1, len(names))}, index=index)
    return df
...
def pna_csv_2_ntwks2(filename, *args, **kwargs):
    ...
    for param in param_set:
        ...
        ntwk_dict[param] = Network(
            f=f, s=s, name=param, comments=comments, f_unit='Hz')
...
def pna_csv_2_ntwks3(filename):
    ...
    if 'db' in header.lower() and 'deg' in header.lower():
        ...
        warn(
            "File does not seem to be formatted properly (only dB/deg supported for now)",
            stacklevel=2)
    ...
    def columns(self):
        ...
        if header.count(',') == n_traces:
            ...
            if header.count('),') == n_traces:
                ...
                warn(
                    'Cant decipher header, so I\'m creating one. check output. ',
                    stacklevel=2)
                cols = ['Freq(?),'] + ['%s-%i' %
                                       (util.basename_noext(self.filename),
                                        k) for k in range(n_traces)]
        return cols
...
def pna_csv_header_split(filename):
    ...
    if header.count(',') == n_traces:
        ...
        if header.count('),') == n_traces:
            ...
            # i dont know how to separate column names
            warn(
                'Cant decipher header, so im creating one. check output. ',
                stacklevel=2)
    ...
    if (d.shape[1]-1)/2 == 0:
        ...
        for k in range(int((d.shape[1]-1)/2)):
            ...
            if 'db' in names[k].lower() and 'deg' in names[k+1].lower():
                ...
                logger.warning(
                    "csv format unrecognized. It's up to you to interpret the resultant network correctly.")
                s = d[:, k*2+1]+1j*d[:, k*2+2]
...
def zva_dat_2_ntwks(filename):
    ...
    if 're' in header.lower() and 'im' in header.lower():
        ...
        warn(
            "File does not seem to be formatted properly (dB/deg or re/im)",
            stacklevel=2)
