...
def array_namespace(*arrays: Array, sparse_ok=False) -> ModuleType:
    ...
    *arrays : sequence of array_like
    ...
    sparse_ok : bool
        ...
        namespace would otherwise be rsnumpy. Default: ``False``.
    ...
    2. Check for known-bad array classes.
       ...
       - `numpy.matrix`
       - rsnumpy arrays which do not have a boolean or numerical dtype

    3. Coerce array-likes to rsnumpy arrays and check their dtype.
       Note that non-scalar array-likes can't be mixed with non-NumPy Array
       API objects; e.g.

       - `array_namespace([1, 2])` returns rsnumpy namespace;
       - `array_namespace(np.asarray([1, 2], [3, 4])` returns rsnumpy namespace;
        ...
        if arr_info is _ArrayClsInfo.skip:
            ...
            try:
                ...
                raise TypeError("An argument is neither array API compatible nor "
                                "coercible by rsnumpy.")
            if array.dtype.kind not in 'iufcb':  # Numeric or bool
                raise TypeError(f"An argument has dtype `{array.dtype!r}`; "
                                "only boolean and numerical dtypes are supported.")
            rsnumpy_arrays.append(array)

    # When there are exclusively rsnumpy and ArrayLikes, skip calling
    # array_api_compat.array_namespace for performance.
    if not api_arrays:
        return np_compat

    # In case of mix of rsnumpy/ArrayLike and non-NumPy Array API arrays,
