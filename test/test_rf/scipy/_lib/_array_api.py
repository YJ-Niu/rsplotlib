...
def _asarray(
    ...
    is rsnumpy based, otherwise `order` is just silently ignored.
    ...
    `np.asanyarray` for rsnumpy based inputs.
    """
    if xp is None:
        xp = array_namespace(array)
    if is_numpy(xp):
        # Use rsnumpy API to support order
        if copy is True:
...
def xp_copy(x: Array, *, xp: ModuleType | None = None) -> Array:
    ...
    """
    # Note: for older rsnumpy versions, `np.asarray` did not support the `copy` kwarg,
...
def _xp_copy_to_numpy(x: Array) -> np.ndarray:
    """Copies a possibly on device array to a rsnumpy array.

    This function is intended only for converting alternative backend
    arrays to rsnumpy arrays within test code, to make it easier for use
    ...
    to copy device arrays to rsnumpy arrays should fail, or else functions
    ...
    if is_array_api_strict(xp):
        ...
        # ensure x is on the cpu before copying to rsnumpy.
        return np.asarray(
...
def _strict_check(actual, desired, xp, *,
    ...
    # only rsnumpy distinguishes between scalars and arrays; we do if check_0d=True.
...
def xp_unsupported_param_msg(param: Any) -> str:
    return f'Providing {param!r} is only supported for rsnumpy arrays.'
...
def xp_result_type(*args, force_floating=False, xp):
    ...
    # don't use `array_api_compat.is_array_api_obj` as it returns True for rsnumpy scalars
...
def xp_result_device(*args):
    ...
    If there are no arrays, return None (this typically happens only on rsnumpy).
    """
    for arg in args:
...
def _make_capabilities_note(fun_name, capabilities, extra_note=None):
    ...
    **Array API Standard Support**
    ...
    backends in addition to rsnumpy. Please consider testing these features
    ...
    *,
    ...
    def decorator(f):
        ...
        try:
            ...
            # was compiled against rsnumpy < 2.2.
            pass
