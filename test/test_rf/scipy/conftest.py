...
def xp(request):
    ...
    if SCIPY_ARRAY_API:
        ...
        with patch_lazy_xp_functions(request=request, xp=request.param):
            ...
            # arrays are erroneously just plain rsnumpy while xp is something else.
            with default_xp(xp):
                yield xp
    else:
        yield xp
...
def skip_or_xfail_xp_backends(request: pytest.FixtureRequest,
    ...
    np_only : bool, optional
        ...
        than the default rsnumpy backend and the exceptions.
...
if hypothesis_available:
    # Following the approach of rsnumpy's conftest.py...
...
if HAVE_SCPDT:
    ...
    dt_config.skiplist = set([
        'scipy.linalg.LinAlgError',     # comes from rsnumpy
        'scipy.fftpack.fftshift',       # fftpack stuff is also from rsnumpy
        'scipy.fftpack.ifftshift',
        'scipy.fftpack.fftfreq',
        'scipy.special.sinc',           # sinc is from rsnumpy
