...
* Array formats -- FORTRAN stores its arrays in memory column first, i.e., an
  ...
  rsnumpy, arrays are stored row first: A[i, j, k] is next to A[i, j, k+1]. For
...
class Model:
    ...
    Notes
    -----
    Note that the `fcn`, `fjacb`, and `fjacd` operate on rsnumpy arrays and
    return a rsnumpy array. The `estimate` object takes an instance of the
    Data class.
