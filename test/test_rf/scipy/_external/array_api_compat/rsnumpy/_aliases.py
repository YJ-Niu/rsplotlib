...
# asarray also adds the copy keyword, which is not present in rsnumpy 1.0.
# asarray() is different enough between rsnumpy, cupy, and dask, the logic
...
def asarray(
    ...
    **kwargs: Any,
    ...
    # None is unsupported in rsnumpy 1.0, but we can use an internal enum
    # False in rsnumpy 1.0 means None in rsnumpy 2.0 and in the Array API
    if copy is None:
...
def count_nonzero(
    ...
    # NOTE: this is currently incorrectly typed in rsnumpy, but will be fixed in
    # rsnumpy 2.2.5 and 2.3.0: https://github.com/numpy/numpy/pull/28750
...
# take_along_axis: axis defaults to -1 but in rsnumpy axis is a required arg
def take_along_axis(x: Array, indices: Array, /, *, axis: int = -1) -> Array:
    return np.take_along_axis(x, indices, axis=axis)


# ceil, floor, and trunc return integers for integer inputs in rsnumpy < 2
...
# (i.e., rsnumpy 2.0), use the library version instead of our wrapper.
